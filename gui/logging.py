...
# code based off of imagine.logging.py

SECONDS_IN_HOUR = 3600
LOG_LEVEL_ENV_VAR = 'iMAGINE_LOG'
BASE_LOGGER_NAME = 'imagine'
EXTENDED_DEBUG = 5
...
def setup_logger(level=logging.INFO, console_output=True, file_output=False,
    ...
    """
    Set up a new logging.Logger for iMAGINE logging

    Calling this method will override any existing handlers, formatters etc.
    attached to the iMAGINE logger. Typically, :func:`get_logger` should be
    used instead, which returns the existing MAGINE logger if it has already
    been set up, and can handle iMAGINE submodule namespaces.
    ...
    Returns
    -------
    A logging.Logger object for iMAGINE logging. Note that otheri MAGINE
    modules should use a logger specific to their namespace instead by calling
    :func:`get_logger`.
    """
    ...
    log.info('Logging started on iMAGINE version {}'.format(__version__))
    if time_utc:
...
def get_logger(logger_name=BASE_LOGGER_NAME, log_level=None,
               **kwargs):
    """
    Returns (if extant) or creates a iMAGINE logger

    If the iMAGINE base logger has already been set up, this method will return
    it or any of its descendant loggers without overriding the settings - i.e.
    ...
    **kwargs : kwargs
        ...
        the iMAGINE logger hasn't been set up yet (i.e. there have been no
    ...
    if BASE_LOGGER_NAME not in logging.Logger.manager.loggerDict.keys():
        ...
        warnings.warn('iMAGINE logger already exists, ignoring keyword '
